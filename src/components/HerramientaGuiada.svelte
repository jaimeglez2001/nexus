...
{#if count !== 9}
  ...
  <article class="ctas flex gap-10 mb-20">
    <button
      on:click={() => renderHuella(0)}
      class="px-8 py-4 border-black text-xl border-2 hover:bg-[--nexusBlue] hover:text-white duration-200 hover:border-[--nexusBlue]"
      >{etapas[count].respuesta[0]}</button
    >
    <button
      on:click={() => renderHuella(1)}
      class="px-8 py-4 border-black text-xl border-2 hover:bg-[--nexusBlue] hover:text-white duration-200 hover:border-[--nexusBlue]"
      >{etapas[count].respuesta[1]}</button
    >
    <button
      on:click={() => renderHuella(2)}
      class="px-8 py-4 border-black text-xl border-2 hover:bg-[--nexusBlue] hover:text-white duration-200 hover:border-[--nexusBlue]"
      >{etapas[count].respuesta[2]}</button
    >
    <button
      on:click={renderVacio}
      class="px-8 py-4 border-black text-xl border-2 hover:bg-[--nexusBlue] hover:text-white duration-200 hover:border-[--nexusBlue]"
      >{etapas[count].respuesta[3]}</button
    >
  </article>
{/if}
